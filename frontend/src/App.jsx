    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="w-80 bg-white border-r relative flex-shrink-0">
        <div className="sticky top-0 h-screen p-8 flex flex-col shadow-sm">
          <h2 className="text-2xl font-black text-indigo-900 mb-10 tracking-tight italic">SAND ART<br/><span className="text-indigo-400 not-italic">STUDIO</span></h2>
      ...
      {/* MAIN CONTENT AREA */}
      <main className="flex-1 p-12 overflow-visible">
        <div className="max-w-4xl mx-auto space-y-16">
            ...
            {!previewUrl ? (
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-8">
                ...
                <div className="flex-1 flex items-center gap-4">
                  ...
                  <button onClick={generateOutlines} disabled={variantsLoading} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-indigo-100 disabled:opacity-50">
          ...
          {/* 2. VARIANT GALLERY (Step 2 & 3 Combined) */}
          {variants.length > 0 && (
            <section className="space-y-6">
              <div className="flex items-center gap-4">
              ...
              <div className="flex flex-row gap-6 overflow-x-auto pb-10 no-scrollbar overflow-y-visible">
                {variants.map((v) => (
                  <div 
                    key={v.id}
                    onClick={() => setSelectedVariant(v)}
                    className={`flex-shrink-0 cursor-pointer border-4 rounded-3xl overflow-hidden bg-white transition-all duration-300 w-64 h-64 relative z-0 hover:z-50 hover:scale-125 shadow-sm hover:shadow-2xl ${
                      selectedVariant?.id === v.id ? 'border-green-500 ring-4 ring-green-100' : 'border-transparent'
                    }`}
                  >
                    <img src={v.url} alt="Variant" className="w-full h-full object-cover" />
                    {selectedVariant?.id === v.id && <div className="absolute bottom-4 right-4 bg-green-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shadow-lg">✓</div>}
                  </div>
                ))}
              </div>
            </section>
          ...
          {selectedVariant && (
            <section className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-gray-100 relative">
              <div className="flex items-center gap-4 mb-10">
                ...
                <h3 className="text-2xl font-black text-gray-900 tracking-tighter uppercase italic flex items-center gap-3">
                  ...
                  <div className="group relative">
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 w-64 p-4 bg-gray-900 text-white text-[11px] rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-2xl leading-relaxed z-[60]">
                      To view and check your .STL file before printing, we recommend the <strong>Microsoft 3D Viewer</strong>.
                      <a href="https://apps.microsoft.com/detail/9nblggh42ths?hl=en-us&gl=CA" target="_blank" rel="noreferrer" className="block mt-2 text-indigo-400 font-bold underline pointer-events-auto">Download 3D Viewer Here</a>
                    </div>
                  </div>
                </h3>
              </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <label className="block">
                      ...
                      <input type="number" step="0.1" value={modelSettings.wallHeight} onChange={(e) => updateSetting('wallHeight', e.target.value)} className="mt-2 w-full p-4 bg-indigo-50 border-none rounded-2xl font-black text-indigo-700 outline-none" />
                    </label>
                    <label className="block">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Wall Width (mm)</span>
                      <input type="number" step="0.1" value={modelSettings.wallThickness} onChange={(e) => updateSetting('wallThickness', e.target.value)} className="mt-2 w-full p-4 bg-indigo-50 border-none rounded-2xl font-black text-indigo-700 outline-none" />
                    </label>
                  </div>
              ...
              <button 
                ...
                className={`w-full mt-10 py-6 rounded-3xl font-black text-xl tracking-tighter shadow-2xl transition-all active:scale-[0.98] ${
                  stlStatus === 'working' ? 'bg-gray-100 text-gray-400' : 'bg-green-500 hover:bg-green-600 text-white'
                }`}
              >
